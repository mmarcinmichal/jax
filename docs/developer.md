...
Moreover, if you require visualization generated by Matplotlib during the tests,
it is necessary to install it separately using the command:
```
pip install matplotlib
```

Additionally, set the LOBPCG_EMIT_DEBUG_PLOTS environment variable to 1 in
the /tests/BUILD file or pass it via the command line. For example, for a Bazel test case:
