...
LogicalResult vector_shape_cast_rule(RewriteContext &ctx, Operation &op,
  ...
  if (layout_in == layout_out && src_ty.getShape().take_back(layout_rank) ==
    ...
    no_op = true;
             ...
             *(src_shape.end() - 2) % layout_out.tiling()[0] == 0 &&
             *(dst_shape.end() - 1) % layout_in.tiling()[1] == 0) {
...
Value copy_one_sublane(RewriteContext &ctx, OpBuilder &builder, Value src_vreg,
                       int src_sl_idx, Value dst_vreg, int dst_sl_idx) {
  if (!dst_vreg) {
  ...
  auto sublanes_mask = builder.create<tpu::CreateMaskOp>(
      src_vreg.getLoc(), VectorType::get(ctx.target_shape, builder.getI1Type()),
  ...
  if (src.implicit_dim() == VectorLayout::ImplicitDim::kNone &&
    ...
    src_tiles_retiled.Each([&](absl::Span<const int64_t> idx, Value *tile) {
      for (int dst_sl_idx = 0; dst_sl_idx < 8; ++dst_sl_idx) {
        ...
        *tile = copy_one_sublane(ctx, builder, src_tiles(src_idx), src_sl_idx,
                                 *tile, dst_sl_idx);
      }
    });
    src = dst;
    ...
    VectorType vreg_f32 =
        VectorType::get(ctx.target_shape, builder.getF32Type());
